...
norm.values <- preProcess(train.df[, 1:32], method=c("center", "scale"))
train.norm.df[,1:32] <- predict(norm.values, train.df[, 1:32])
valid.norm.df[,1:32] <- predict(norm.values, valid.df[, 1:32])
organic.norm.df[,1:32] <- predict(norm.values, organic2.df[, 1:32])
...
library(FNN)
organic.knn <- knn(train = train.norm.df[, 1:32], test = valid.norm.df[1, 1:32], cl = train.norm.df[, 33], k = 3)
...
#KNN accuracy
accuracy.df <- data.frame(k =  seq(1, 1000, 1), accuracy = rep(0, 100)) #inialize a data frame

for(i in 1:1000) {
  organic.knn.pred <- knn(train.norm.df[, 1:32], valid.norm.df[, 1:32], cl = train.norm.df[,33], k=i)
  accuracy.df[i, 2] <- confusionMatrix(as.factor(organic.knn.pred), as.factor(valid.norm.df[,33]))$overall[1]
} 
accuracy.df
