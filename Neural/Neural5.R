...
organic.nn.pred <- compute(organic.nn, data.frame(valid.norm.df$DemAffl, valid.norm.df$DemAge, valid.norm.df$PromTime, valid.norm.df$Unknown, valid.norm.df$Female,
                                                  valid.norm.df$F., valid.norm.df$North, valid.norm.df$Border,  valid.norm.df$Silver))
...
#confusion matrix
cmatrix <- confusionMatrix(as.factor(ifelse(organic.nn.pred$net.result > 0.5, 1, 0)), as.factor(valid.norm.df$TargetBuy))
cmatrix

#determine best cutoff
accT = c()
for(cutoff in seq(0, 1, 0.05)){
  cm <- confusionMatrix(as.factor(ifelse(organic.nn.pred$net.result > cutoff, 1, 0)), as.factor(valid.df$TargetBuy))
