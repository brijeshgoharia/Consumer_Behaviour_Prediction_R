...
row.has.na <- apply(organic1.df, 1, function(x){any(is.na(x))})
sum(row.has.na)
#Checking the Dummy Formation
#contrasts(organic1.df$DemClusterGroup)
#dummy variables, need variables in numeric
...
#organic.nn.step <- step(organic.nn, direction = "both")


#plot neural network
plot(organic.nn, rep = "best")
